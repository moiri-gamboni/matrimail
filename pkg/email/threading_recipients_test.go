...
//
// The concrete failure: Alice mails Moïri and Cc's a funder, then sends a
// Moïri-only follow-up in the same thread. LastCc still held the funder, while
...
func TestAddToExistingThread_RecipientsAreNotSticky(t *testing.T) {
	...
	tm.addToExistingThread(thread, &ParsedEmail{
		...
		To:          []string{"moiri@apartresearch.com"},
...
func TestAddToExistingThread_ToIsNotSticky(t *testing.T) {
	...
	tm.addToExistingThread(thread, &ParsedEmail{
		...
		From:      "alice@example.com",
		To:        []string{"moiri@apartresearch.com", "bob@example.com"},
	})
	tm.addToExistingThread(thread, &ParsedEmail{
