...
- **`email/`** — protocol-agnostic email logic: RFC 5322 threading (`threading.go`), MIME processing (`processor.go` — large, holds message conversion, tracking-pixel filtering, participant extraction), media upload and inline-image extraction (`media.go`), outbound assembly (`outbound.go`), and the sender abstraction (`sender.go` + factory + Gmail/SMTP/Graph implementations).
...
- **`beeper/`** — client for the few Beeper Desktop API calls the Beeper state sync makes (account lookup, chat list, archive, read/unread).
...
- **The user's own mail never becomes reply context.** `ProcessParsedEmail` marks a message `Outbound` when its mailbox/label is a Sent one or its From is one of the account's own addresses (the `AliasResolver`). An outbound is threaded and bridged as `IsFromMe`, but `recordReplyContext` leaves the thread's `Last*` inbound fields alone, so replies answer the other party. The merge in `portal_metadata.go` and `fillInboundContext` keep that context when an outbound lands on a skeleton thread after the cache lost it.
- **Gmail `users.history.list` takes one `labelId`,** and the client's `LabelId` setter replaces rather than appends: the poller lists each monitored label separately and merges by history record ID. Take a message's mailbox from the fetched message's labels, never from the history record, whose messages carry only `id`/`threadId`.
- **Beeper state sync is opt-in** (`network.beeper_sync`, `connector/beeper_sync.go`). Beeper never tells the bridge that a chat was archived or read, so that side is read from the Beeper Desktop API each interval; the Gmail side comes from the history poller's `OnThreadsChanged`, which makes one extra history.list call with no label filter (an archived message no longer matches `labelId=INBOX`, and UNREAD is not monitored). The `matrimail_beeper_sync` table holds each thread's last agreed state; a side that differs from it has changed, and writing the merged state back to both sides and to the table is what stops the bridge's own change from echoing. Beeper write responses show the chat as it was before the write, so writes are confirmed by re-reading.
- **Every media upload goes through `uploadAttachment` (`email/media.go`) with the portal's room ID.** bridgev2's `UploadMedia` encrypts only when handed an encrypted room's ID and then returns the mxc URI inside an `EncryptedFileInfo` (for `content.File`), not as a URL; with an empty room ID it uploads plaintext. For the same reason inline images never go into `formatted_body` as `<img src="mxc://…">`: they are sent as separate image events. Gmail-API attachments carry a deferred download (`EmailAttachment.load`, `users.messages.attachments.get`) that runs only at conversion, after the size check.
...
This path costs the user a long-lived full-mailbox credential in their room history, which no redaction reliably removes. Every surface that mentions it says so: the subcommand list, the usage reply, the login instructions and `docs/gmail-oauth-setup.md`. Keep it that way if you touch any of them, and don't present it as equivalent to an SSH port-forward.
